...
void LCM::afficherDetection(double distance1, double distance2, double distance3, const char *categorie1, const char *categorie2, const char *categorie3)
{

	clear();
	char dist1[5];
	sprintf(dist1, dtostrf(distance1, 3, 1, "%f\0"));

	char dist2[5];
	sprintf(dist2, dtostrf(distance2, 3, 1, "%f\0"));

	char dist3[5];
	sprintf(dist3, dtostrf(distance3, 3, 1, "%f\0"));

	write(dist1, 0);
	write("m", cp_strlen(dist1));
	write(dist3, LCM_FW_HALF_CH - cp_strlen(dist3) - 1);
	write("m", LCM_FW_HALF_CH - cp_strlen(dist3) + 2);
	affichageCatetDist2(categorie1, categorie2, categorie3, dist2);
}

void LCM::affichageCatetDist2(const char *categorie1, const char *categorie2, const char *categorie3, const char *dist2)
{

	if (categorie1 == "OK")
	{
		write(categorie1, LCM_FW_HALF_CH + 1);
	}
	else
	{
		write(categorie1, LCM_FW_HALF_CH);
	}

	if (categorie2 == "OK")
	{
		write(categorie2, LCM_FW_HALF_CH + LCM_FW_HALF_CH / 2 - 1);
		write(dist2, LCM_FW_HALF_CH / 2 - 2);
		write("m", LCM_FW_HALF_CH / 2 + 1);
	}
	else
	{
		write(categorie2, LCM_FW_HALF_CH + LCM_FW_HALF_CH / 2 - 2);
		write(dist2, LCM_FW_HALF_CH / 2 - 2);
		write("m", LCM_FW_HALF_CH / 2 + 1);
	}
	...
	if (categorie3 == "OK")
	{
		write(categorie3, LCM_FW_TOT_CH - cp_strlen(categorie3) - 1);
	}
	else
	{
		write(categorie3, LCM_FW_TOT_CH - cp_strlen(categorie3));
	}
}

void LCM::afficherManoeuvre(uint8_t numero)
{
	clear();
	char str[20];
...
void LCM::afficherAucuneManoeuvre()
{

	clear();
	write("Combinaison", 0);
	write("non evaluee", LCM_FW_HALF_CH);
}
